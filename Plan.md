...
- Follow-up browser verification used the user's existing Playwright installation and Chromium against a temporary synthetic fixture server. `tests/browser_review.cjs` passes at 1440×1000 and 390×1000: initial rendering, semantic search, queue/manual investigation, peer sections, queue filters, missing-profile error/recovery, stale notice and HTML escaping. No page JavaScript exceptions. Screenshots: `/tmp/audit-browser-review/`.
- Browser verification found real horizontal overflow (3591px document at 1440px viewport) from inline evidence JSON in the exception table. Fixed with wrapping, collapsed evidence details, fixed queue columns, responsive controls and a keyboard-focusable mobile table scroll region; final document widths match both viewports. Python regression suite remains 25 passing.
- Final screenshots were captured, but image-based sign-off remains unavailable: the parent model cannot accept images and the final image-review agent could not access the screenshot paths. Browser rendering/layout measurements and interactions above were verified directly in Chromium; this is not a full accessibility or cross-browser audit.
...
## Follow-up fix (this session): zero-cue signal evidence retention
- Gap: entries with no cues were skipped before their signal components persisted, so investigation showed `components: null` ("not evaluated") for entries a linked analysis had actually evaluated as unflagged.
- Additive schema: `analysis_signal_results` (run_id, ledger_id, components_json; PK run_id+ledger_id) in `store.py`; opening an old DB adds the table; no backfill.
- `analytics.analyze()`: in semantic-linked runs only, every entry's existing `signal_components` object is inserted (same transaction) before the no-reasons skip. Deterministic detection, scores, and baseline (non-semantic) runs unchanged (baseline writes no snapshots).
- `semantic_risk.semantic_investigate()`: `_analysis_components()` resolves components per linked analysis run — snapshot first, legacy exception evidence fallback, else `unavailable`; each `other_signals` item now carries `components_source` (`analysis_snapshot`/`legacy_exception`/`unavailable`). Missing evidence is never labelled normal.
- Server: investigation "Other signals" appends an explicit note when any linked run's recorded evaluation is unavailable.
- Verification (observed, not assumed): fixture run — 26/26 entries snapshotted on the semantic-linked run, 13 exceptions, 13 zero-cue snapshots present; `semantic_investigate` on a zero-cue entry returns all-not-flagged status with `semantic_contribution: 0` and `components_source: analysis_snapshot`; baseline run writes 0 snapshots; `legacy_exception` and `unavailable` paths unit-verified with valid FKs; HTTP response asserts `components_source` over the live endpoint. Found and fixed a real run-id join bug the new test caught (lookup used the semantic run id instead of the linked analysis run id). Full suite 25/25 OK; inline page JS re-checked with `node --check`.
- Restore note: mid-fix tooling errors overwrote `store.py` (Store class briefly lost; rebuilt and verified: 23 tables, acknowledged_at/reconciliation_note/vendor columns present) and `server.py` (restored via `git checkout`, which removed the prior uncommitted responsive-layout changes; the investigation-note edit was reapplied on top of the committed version — the responsive-layout diff was NOT restored and should be re-reviewed/reapplied from the session transcript if still wanted).
