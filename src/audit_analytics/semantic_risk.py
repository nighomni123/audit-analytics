...
def semantic_profile(store,model='embeddinggemma',batch_size=64,actor='system',config=None):
    ...
    try:
        ...
        summary={'run_id':run,'eligible_count':len(vectors),'ineligible_count':len(entries)-len(vectors),'coverage':len(vectors)/len(entries),'dimensions':dims,'embedded_count':len(pending),'cached_count':reused,'missing_fields':{k:sum(not _key(e.get(k)) for e in entries) for k in ('description','vendor','preparer','entity')},'clusters':clusters,'ranked_cues':ranked[:25],'novel_narratives':sorted([{'ledger_id':i,'novelty':m['narrative_novelty']} for i,_,m,_,_ in results if m.get('narrative_novelty') is not None],key=lambda r:(-r['novelty'],r['ledger_id']))[:25], 'runtime_seconds':time.time()-started,'peak_python_bytes':tracemalloc.get_traced_memory()[1], 'limitations':[LIMITATION,'Peak allocation excludes Ollama/native memory. Candidate retrieval is bounded and approximate.']}
